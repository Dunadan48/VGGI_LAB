...
function CreateSurfaceData() {
    ...
    for (let uc = 0.25; uc <= 1; uc += STEP) {
        for (let vc = 0; vc < Math.PI * 2; vc += STEP) {
            // const v1 = RichmondMinimalSurface(uc, vc);
            const v1 = cylindrical(uc, vc);
            // const v2 = RichmondMinimalSurface(uc + STEP, vc);
            const v2 = cylindrical(uc + STEP, vc);
            // const v3 = RichmondMinimalSurface(uc, vc + STEP);
            const v3 = cylindrical(uc, vc + STEP);
            // const v4 = RichmondMinimalSurface(uc + STEP, vc + STEP);
...
function RichmondMinimalSurface(u, v) {
    let x = xf(u, v);
    let y = yf(u, v);
    let z = u;
    const COEF = 2;
...
function xf(u, v) {
    return ((-3 * u - u ** 5 + 2 * u ** 2 * v ** 2 + 3 * u * v ** 4) / (6 * (u ** 2 + v ** 2)))
}

function yf(u, v) {
    return ((-3 * v - 3 * u ** 4 * v - 2 * u ** 2 * v ** 3 + v ** 5) / (6 * (u ** 2 + v ** 2)))
}
